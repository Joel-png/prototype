...
func look_at_position(pos, delta):
	var rotation_speed = 2.0
	var target_position = pos
	...
	#fix_rotation(eye.rotation.y, angle_yaw)
...
func fix_rotation(rot_from, rot_to):
	counter += 1
	if counter > 1000:
		print("NOT")
		print(rot_from)
		print(rot_to)
		counter = 0
	if eye.rotation.y > 0 and rot_to < 0:
		print(rot_from)
		print(rot_to)
		eye.rotation.y -= deg_to_rad(360)
	elif eye.rotation.y < 0 and rot_to > 0:
		print(rot_from)
		print(rot_to)
		eye.rotation.y += deg_to_rad(360)
